...
bool BreadthFirstSearch2 (long grafo_residual[][MAX], int S, int T, int N ){
	aresta aux;
	int ansSize;
	int v1,v2, i, j;
	int caminho[100];
	...
	while(!listaVazia(lista, N)){
		...
		for (v2=0; v2<N; v2++){
			if(grafo_residual[v1][v2] == 0 && visitado[v2] == 0) {
				aux.a = v1+1;
				aux.b = v2+1;
				ans.push_back(aux);
			}
			if(visitado[v2] == 0 && grafo_residual[v1][v2] > 0){
				for(int i=0; i<ans.size(); i++){
					if(ans.at(i).b == v2+1) ans.at(i).a = -1;;
				}
				lista[proximoVerticeLista(lista, N)] = v2;
				caminho[v2] = v1;
				visitado[v2] = 1;
...
int FordFulkerson(long grafo[][MAX], int S, int T, int N, long grafo_residual[][MAX]){
    int v1, v2;
...
int main() {
	...
	while(n!=0){
        ...
        FordFulkerson(mat, 0, 1, n, matRes);
        BreadthFirstSearch2(matRes, 0, 1, n);
        ...
        for(int i=0; i< aSize; i++){
        	if(ans.at(i).a == -1);
        	else if(i+1!=aSize) printf("%d %d\n", ans.at(i).a, ans.at(i).b);
        	else printf("%d %d", ans.at(i).a, ans.at(i).b);
        }
        ans.clear();
